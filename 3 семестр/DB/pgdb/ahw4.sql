...
VALUES ('Сидоров',
        ...
        md5('123456'))
ON CONFLICT (email)
    DO UPDATE SET (name, email, phone, password) = (excluded.name, excluded.email, excluded.phone, excluded.password);

INSERT INTO country(id, name)
VALUES (DEFAULT, 'Россия');

INSERT INTO regions(name, country_id)
VALUES ('Центральный', 1);

INSERT INTO city(name, region_id)
VALUES ('Москва', 1);

INSERT INTO adress(street, house, flat, city_id)
VALUES ('Ленина', 12, 3, 1),
...
SELECT * FROM ingredients WHERE cost <= 350;

-- 3. Изменить их цену на стоимость на равную 350.
UPDATE ingredients SET cost = 350 WHERE cost < 350;

-- 4. Удалить несколько произвольных записей из таблицы характеризующих заказ.
DELETE FROM order_content WHERE order_id = 7;
DELETE FROM order_content WHERE order_id = 8;
DELETE FROM order_content WHERE order_id = 9;